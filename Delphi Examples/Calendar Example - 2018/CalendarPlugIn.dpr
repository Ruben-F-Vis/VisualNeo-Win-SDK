...
BEGIN
  ...
  IF VariableName > '' THEN
    nbSetVar( PChar( VariableName ), PChar( DateToStr( EncodeDate( Year, Month, Day ) ) ) );
END;
...
FUNCTION FindCalendarControl( RectangleName : STRING ) : INTEGER;
VAR I : INTEGER;
BEGIN
...
FUNCTION RemoveCalendarControl( RectangleName : STRING; ShowErrors : BOOLEAN ) : BOOLEAN;
VAR Found : INTEGER;
BEGIN
  Result := FALSE;
  TRY
    ...
    IF Found > -1 THEN
      BEGIN
        ...
        IF TNeoCalendar( CalList.Items[Found] ).VariableName > '' THEN
          nbSetVar( PChar( TNeoCalendar( CalList.Items[Found] ).VariableName ), '' );
    ...
    ELSE IF ShowErrors THEN
      nbSetVar( '[LastError]', PChar( 'There is no calendar control attached to an object named '+RectangleName+'.' ) );

  EXCEPT ON E : Exception DO
    nbSetVar( '[LastError]', PChar( E.Message ) );
  END;
END;

FUNCTION SetCalendarControl( RectangleName, NewDate : STRING ) : BOOLEAN;
VAR Found   : INTEGER;
    M, D, Y : WORD;
BEGIN
  Result := FALSE;
  TRY
    ...
    IF Found > -1 THEN
      BEGIN
        ...
        DecodeDate( StrToDate( NewDate ), Y, M, D );
    ...
    ELSE nbSetVar( '[LastError]',
      PChar( 'There is no calendar control attached to an object named '+RectangleName+'.' ) );
...
FUNCTION CreateCalendarControl( RectangleName, InitialDate, VariableName : STRING ) : BOOLEAN;
      ...
      BEGIN
        ...
        IF ObjHandle <> 0 THEN
          BEGIN
            ...
            IF InitialDate > '' THEN
              BEGIN
                DecodeDate( StrToDate( InitialDate ), Y, M, D );
                Cal.Year  := Y;
                Cal.Month := M;
                Cal.Day   := D;
              END;
            ...
            Result := TRUE;
          END
        ELSE nbSetVar( '[LastError]', PChar( 'An object named '+RectangleName+' does not exist.' ) );
          ...
          TRY
            ...
            IF EditForm1.ShowModal = mrOK THEN
              BEGIN
                ...
                SetStr( Params[1], DateToStr( EditForm1.DateTimePicker1.Date ) );
